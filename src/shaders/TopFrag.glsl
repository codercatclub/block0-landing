...
void main() {
  ...
  float distToCenterY2 = 1.0 - min((vWorldPos.y-4.9)-progressY, 1.0);

  if(distToCenter * progress > 1.0) {
    // float n = round(-0.06*progressY + 0.0*abs(1.0/vWorldPos.y) + -0.1 + cnoise(vec2(0.2*vWorldPos.xy) + offset));
    float n = round(0.07*abs(progressY) + 1.0*(1.0-distToCenterY2) + -0.1 + cnoise(vec2(0.2*vWorldPos.xy) + offset));
    shouldColor = n;
  }
